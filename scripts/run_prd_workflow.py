    ...
    if node_name == "bake_off":
        ...
        if payload.get("stop_reason") == "no_runnable":
            ...
            return (
                ...
                "与 candidates（显示「（无）」表示一个都不能跑）。请回复本机已接入候选的序号或名称改选；"
        ...
        return (
            ...
            "实时单价与取数来源、接入状态），candidates 是本机已接入、本次可实跑的候选。"
            "注意：实跑会真实产生多次模型调用（候选数 × 考题数，产生真实成本与延迟）。"
            "请三选一：① 回复候选序号（如「1」或「1和3」）或候选名（如「deepseek-chat」）"
            "指定跑哪几个；② 回复「全部」跑本机已接入的全部候选；"
            "③ 回复「跳过」/「先用默认」跳过实跑（按默认 DeepSeek 记推荐，不调任何模型）。"
            "未接入的候选会写进结论并标「需接入后验证」，不实跑、不编造数据。"
        )
