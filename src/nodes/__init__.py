...
# launch_plan 发布计划节点（10 节点）：工单确认门确认分支走
#     launch_plan（route 返回 "artifact_persist" 语义值映射到 launch_plan 节点）。
# launch_confirm 发布计划确认门（11 节点）：
...
from nodes.refine import make_requirement_refine  # 需求修订整合节点
from nodes.prd import make_prd_generation
from nodes.review import make_prd_review  # PRD 评审门节点
from nodes.issues import (  # 拆研发工单 + 工单确认门
    make_issue_confirm,
    make_issue_splitting,
)
from nodes.launch_plan import (  # 发布计划 + 发布计划确认门
    make_launch_confirm,
    make_launch_plan,
)
from nodes.feasibility import (  # 判断需求与 AI 的边界 + 确认AI可行性门
    make_feasibility_check,
    make_feasibility_confirm,
)
from nodes.eval_design import (  # 设计评测体系 + 确认评测体系门
    make_eval_confirm,
    make_eval_design,
)
from nodes.eval_run import (  # 第 8 段：构建期跑评测
    eval_gate,  # 第 8 段后半：判定 + 停等
    make_eval_run,
)
from nodes.bake_off import (  # 第 6 段：对比选型模型
    make_bake_off,
)
from nodes.readiness_assessment import make_readiness_assessment  # 就绪度打分
...
def build_nodes(deps: NodeDeps) -> dict:
    ...
    return {
        ...
        # 评审门插在 PRD 生成之后，条件边由 graph.py 装配
        ...
        # 工单确认门（HITL，不调模型），三分支条件边由 graph.py 装配
        "issue_confirm": make_issue_confirm(deps),
        # 第 8 段构建期跑评测（仅 AI 核心需求经过）：
