# 纵切联调 - 产物持久化节点（artifact_persist，确定性）
# 产物改 Markdown 原生：
...
def make_artifact_persist(deps):
    ...
    def artifact_persist(state: dict) -> dict:
        ...
        # 为空（理论上评审门必有输出）容错跳过，不阻断 PRD/洞察落盘。
        artifacts_dict = {
            "prd": str(prd_path),
        ...
        # 5. 发布计划：launch_plan 非空时渲染 launch_plan.md.j2 落 .md；
        ...
        # 5b. 就绪度打分报告：readiness_assessment 非空时渲染并落 .md；
        ...
        # 产物落 .md：prd_markdown 原文 + insights.md.j2 渲染
        # 新增评审报告 review.md.j2 渲染落盘 + artifacts["review"]
        # 新增工单清单 issues.md.j2 渲染落盘 + artifacts["issues"]

    return artifact_persist


# 改落 Markdown：PRD 原文 .md / 洞察 insights.md.j2 .md
