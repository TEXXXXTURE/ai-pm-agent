# 纵切联调 - PRD 阶段节点（prd_generation）
# 创意放松 - 去除机器 guard 自动打回，self_checks 用默认空列表（NodeRunner 自动跳过自检）；
#     质量引导下沉到 prompt 的"资深 PM 质量标杆"自检。guards/prd_checks.py 保留备用，本节点不再挂载。
# 产物改 Markdown 原生 - 不再 load_schema / 不再产出 JSON sections+HTML 片段，
...
def make_prd_generation(deps):
    ...
    def prd_generation(state: dict) -> dict:
        ...
        # 不传 output_schema（NodeSpec 默认 None）、不挂 self_checks；
...
# nodes/prd.py 改走 run_text 文本通道，输出 prd_markdown
# 按 ai_core 选 ai-native / 普通 PRD 模板
