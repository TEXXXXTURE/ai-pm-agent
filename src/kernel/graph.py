...
# 插入 launch_plan 发布计划节点：issue_confirm 确认分支改走
#     launch_plan（route 返回 "artifact_persist" 语义值映射到 launch_plan 节点）。
# 插入 launch_confirm 发布计划确认门：launch_plan 产出后先进确认门，
...
# requirement_confirm 与 needs_discovery 之间插
...
def build_graph(deps: Any, db_path: str | None = None) -> Any:
    ...
    # 放函数内导入可避免 kernel <-> nodes 循环导入。
    from nodes import build_nodes
    from nodes.review import route_after_review  # 评审门条件边
    from nodes.issues import route_after_issue_confirm  # 工单确认门条件边
    from nodes.launch_plan import route_after_launch_confirm  # 发布计划确认门条件边
    from nodes.exploration import (  # 挖需求后分流条件边
        route_after_needs_discovery,
    )
    from nodes.feasibility import (  # 判断需求与 AI 的边界路由
        route_after_feasibility_confirm,
    )
    from nodes.eval_design import (  # 确认评测体系门条件边
        route_after_eval_confirm,
    )
    from nodes.eval_run import (  # 构建期跑评测条件边
        route_after_eval_gate,  # 由 route_after_eval_run 改名
    )
    from nodes.bake_off import (  # 第 6 段对比选型条件边
        route_after_bake_off,
    )
    from nodes.refine import (  # 需求修订整合节点条件边
    ...
    # PRD 生成后先进评审门；打回回 prd_generation 重写（最多 3 轮），
    # 通过/带警告通过/第 3 轮强制放行则落盘
    graph.add_edge("prd_generation", "prd_review")
    graph.add_conditional_edges(
        "prd_review",
        route_after_review,
        # 通过分支去 issue_splitting 拆单（直连落盘，插确认门）
        # 第 5 段三态：非 reject 且 ai_core=True
    ...
    # 第 5 段：评测体系起草后先进确认门（HITL）
    graph.add_edge("eval_design", "eval_confirm")
    graph.add_conditional_edges(
        "eval_confirm",
        route_after_eval_confirm,
        # 第 6 段：确认（route 返回语义值 "issue_splitting"）
        ...
        # 缺 verdict（没有答复）不再兜底放行，路由回本节点继续停等。
        {
            "eval_design": "eval_design",
    ...
    graph.add_conditional_edges(
        ...
        # 第 8 段：确认且 ai_core=True 时 route 返回
        # "eval_run"，先跑构建期评测；普通轨仍返回 "artifact_persist" 直达 launch_plan。
        # 没有工单草案（无答复）不再兜底落盘，路由回本节点继续停等。
        {
            "issue_splitting": "issue_splitting",
            "prd_generation": "prd_generation",
    ...
    # launch_plan 产出后先进就绪度打分，再进发布计划确认门（HITL）
    ...
    graph.add_conditional_edges(
        ...
        # 没有发布计划草案（无答复）不再兜底落盘，路由回本节点继续停等。
        {
