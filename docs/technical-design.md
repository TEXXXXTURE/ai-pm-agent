...
> ⚠️ **时效声明（S031，2026-09-12）**：本文第二节 State、第五节 G1-G9 映射、第六节图装配、第七节 M1-M10 拆解说的是 S007-S008 开建时的规划形态，与当前代码已有差异（实际 11 节点三门、Markdown 产物、评审/工单/发布计划三条流水线建成）。**工作流顺序、节点规划与新增门以 [workflow-design.md v3.0](workflow-design.md)（AI PM 融合工作流 12 段）为准**；实现新节点时按 v3.0 第八节实施队列分块更新本文对应章节，不再整体回写。
