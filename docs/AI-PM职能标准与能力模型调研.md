...
> 日期：2026-09-15（S046）｜调研人：主 Agent（自研结论 + 外部证据）
> 声明：本文第三节「职能标准」与第四节「对本项目的要求」是**本项目的自研结论**；一、二节是外部证据摘录，逐条标了出处。未取到全文的来源在第六节标注。
...
该框架的一句立场：**「Builders over coordinators」——2026 年不招只协调的人。** 注意：这是**招聘市场口径**，里面「公开构建/粉丝量」之类的信号属于个人品牌证据，不能直接搬成产品要求；但它对「动手验证优先于纸面推演」的定价，与本项目「探针真跑」的取向一致。

### 2.5 AI PM 工作流、交付物与发布门（可执行的工程侧范本）
...
| B3 | 同一批考题横跑候选模型，报质量/成本/延迟三维；结论由数据得出，不由偏好得出 |
...
判断一个 AI PM 或一个团队是否「优秀」，看的是**有多少项到 L3、有没有项到 L4**；只看 L1/L2 的，就是 Playbook 说的「demo 做得出来、产品做不出来」。这份等级也是本项目的产品目标：**把用户从 L1/L2 顶到 L3**，并在有数据后提供 L4 的载体。

---

## 四、对本项目（AI PM Agent）的要求

把上面 14 项落到已建成的 12 段流水线上，得到 12 条硬要求。状态列是 2026-09-15 的代码实况（依据 `docs/workflow-design.md` 第七节与 `.trae/memory/项目逻辑导图.md`）。
...
| R3 | 模型能力必须实测判定，不吃榜单、不做纸面推演 | 第 2 段探针（三方对照 + ReAct 真跑 ≤8 轮 + 证据回填） | ✅ 已达标（S043/S045 真实 5 探针全跑） |
...
| R7 | 选型用同一批考题横跑，报三维（质量/成本/延迟），结论代码硬判 | 第 6 段 bake_off（调 Promptfoo） | ✅ 已建，真机横跑待复跑（S039 首跑数据被阅卷模型缺失污染） |
...
R14（表述像 PM）属内容打磨，按项目口径留到有使用数据后再迭代。
